...
public class Login {
    ...
    void setNama(){
        try {
            ...
            String sql = "select nama_teknisi, email, teknisi.id_teknisi from user INNER JOIN teknisi USING (id_teknisi) where username='"+user+"' AND password='"+pass+"'";
            rs = stmt.executeQuery(sql);
            if (rs.next()) {
                if (rs.getString(1) != null) {
                    namaTeknisi = rs.getString(1);
                    email = rs.getString(2);
                    idTeknisi = rs.getString(3);
                    login();
                }
            }
            stmt.close();
